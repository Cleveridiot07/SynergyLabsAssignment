...
export default function CreateUserForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to create user'); // Set error state if the request fails
      setToast({ message: 'Failed to create user', visible: true, type: 'error' }); // Show error toast
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* User inputs (name, username, email, etc.) */}
          <div>
